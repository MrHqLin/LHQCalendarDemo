...
#pragma mark 获取当前年月日,dateStyle为时间的格式  例如:yyyy/MM/dd HH:mm:ss
+(NSString *)nowDate:(NSString *)dateStyle
{
    NSDate * date = [NSDate date];
...
#pragma mark 时间戳转字符串
+(NSString *)timeIntervalToString:(NSInteger)timeInterval format:(NSString *)format
{
...
#pragma mark 字符串转时间戳
+(NSInteger)stringToTimeInterval:(NSString *)string format:(NSString *)format
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:format];
